...
function getWebpackConfig(fromDir) {
  ...
  if (webpackConfigCache[pathname]) return webpackConfigCache[pathname];

  if (!_fsPlus2.default.existsSync(pathname)) throw new Error('Webpack config does not exists at ' + pathname + '.');
